...
export const createTransactionController = async (req: Request, res: Response) => {  
  ...
  try {
    ...
    if (sourceAccountId === targetAccountId) return res.status(405).json({error: 'Not allowed same source/target account'});
    ...
    const transactionId = await createTransaction({ sourceAccountId, targetAccountId, amount });

    res.status(201).json({ message: `Transaction with ${JSON.stringify(transactionId)} successful` });
  } catch (error) {
